...
/**
 * Detects Gap Fill Long (Gap Down) signals (SM-25).
 *
 * <p>Triggered when today's opening price is <strong>below</strong> the previous
 * session's close by 0.5%–3.0% (a downward gap).  The expectation is that
 * the price will fill the gap by returning to the previous close (LONG direction).
 *
 * <p>Conditions:
 * <ol>
 *   <li>Gap % = (prevClose − todayOpen) / prevClose × 100 is in [0.5, 3.0]</li>
 ...
 *   <li>SL     = first candle low (gap extends further → trade invalidated)</li>
 *   <li>T1     = prevClose (the gap-fill level)</li>
 *   <li>T2     = prevClose + risk (symmetric extension)</li>
 * </ul>
 *
 * <p>The signal is valid for 15 minutes after generation.  If the price
 * moves &gt;0.5% further in the gap direction before {@code valid_until},
 * the trade should be abandoned (monitored externally).
 */
@Component
public class GapFillLongDetector implements SignalDetector {
    ...
    public GapFillLongDetector(CandleRepository candleRepository) {
    ...
    public Optional<Signal> detect(Stock stock, List<Candle> todayCandles,
        ...
        // Resolve session start (the first candle's time, truncated to the minute, is our anchor)
    ...
    Optional<Signal> detectWithPrevClose(Stock stock, List<Candle> todayCandles,
        ...
        for (Candle candle : todayCandles) {
            ...
            // Condition 1: gap % in [0.5, 3.0] (gap-down)
            BigDecimal gapPct = prevClose.subtract(todayOpen)
    ...
    private Optional<Signal> buildSignal(Stock stock, Candle candle,
        ...
        BigDecimal stopLoss = candle.getLow().setScale(2, RoundingMode.HALF_UP);
        ...
        BigDecimal t2 = t1.add(risk).setScale(2, RoundingMode.HALF_UP); // symmetric extension

        int regimeModifier = regime != null ? parseModifier(regime) : 0;
        int confidence = Math.max(0, Math.min(100, 50 + regimeModifier));

        Signal signal = new Signal(
                stock, SignalType.GAP_FILL_LONG, SignalDirection.LONG,
    ...
    @Override public String detectorName() { return "GapFillLongDetector"; }
    @Override public SignalType signalType() { return SignalType.GAP_FILL_LONG; }
