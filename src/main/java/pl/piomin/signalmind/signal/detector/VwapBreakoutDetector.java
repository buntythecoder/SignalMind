...
/**
 * Detects VWAP Breakout signals (SM-23).
 *
 * <p>Six conditions must ALL be satisfied on the current candle:
 * <ol>
 *   <li>Previous close was below VWAP (price was under VWAP)</li>
 *   <li>Current close is above VWAP (price crossed above VWAP)</li>
 *   <li>Volume &ge; 2.0&times; time-slot baseline (institutional conviction)</li>
 *   <li>RSI(14) between 40 and 70 (not oversold, not overbought)</li>
 *   <li>Close is within 0.5% of VWAP (entry is close to VWAP)</li>
 *   <li>Market regime is NOT {@code TRENDING_DOWN}</li>
 * </ol>
 *
 * <p>Shares a combined daily cap of 3 signals with {@link VwapBreakdownDetector}
 * (VWAP_BREAKOUT + VWAP_BREAKDOWN combined per stock per session).
 */
@Component
public class VwapBreakoutDetector implements SignalDetector {

    private static final ZoneId IST = ZoneId.of("Asia/Kolkata");

    private static final BigDecimal VOLUME_MULT    = new BigDecimal("2.0");
    private static final BigDecimal RSI_MIN        = new BigDecimal("40");
    private static final BigDecimal RSI_MAX        = new BigDecimal("70");
    ...
    public Optional<Signal> detect(Stock stock, List<Candle> todayCandles,
        ...
        // Condition 6: regime must NOT be TRENDING_DOWN
        ...
        for (int i = 1; i < todayCandles.size(); i++) {
            ...
            // Skip synthetic candles — no real tick, cannot confirm breakout
            ...
            // Condition 1: prev close was below VWAP
            if (prevClose.compareTo(prevVwap) >= 0) continue;

            // Condition 2: curr close is above VWAP
            ...
            // Condition 4: RSI between 40 and 70
    ...
    private Optional<Signal> buildSignal(Stock stock, Candle curr, BigDecimal vwap, String regime) {
        ...
        // SL = vwapLower (if available) else VWAP × (1 − 0.5%)
        BigDecimal stopLoss = curr.getVwapLower() != null
                ? curr.getVwapLower().setScale(2, RoundingMode.HALF_UP)
                : vwap.multiply(BigDecimal.ONE.subtract(VWAP_PROX_PCT.divide(HUNDRED, 6, RoundingMode.HALF_UP)))
                      .setScale(2, RoundingMode.HALF_UP);

        BigDecimal risk = entry.subtract(stopLoss);
        if (risk.compareTo(BigDecimal.ZERO) <= 0) return Optional.empty();

        BigDecimal t1 = entry.add(risk.multiply(ONE_POINT_FIVE)).setScale(2, RoundingMode.HALF_UP);
        BigDecimal t2 = curr.getVwapUpper() != null
                ? curr.getVwapUpper().setScale(2, RoundingMode.HALF_UP)
                : entry.add(risk.multiply(TWO)).setScale(2, RoundingMode.HALF_UP);

        int regimeModifier = regime != null ? parseModifier(regime) : 0;
        int confidence = Math.max(0, Math.min(100, 50 + regimeModifier));

        Signal signal = new Signal(
                stock, SignalType.VWAP_BREAKOUT, SignalDirection.LONG,
    ...
    @Override public String detectorName() { return "VwapBreakoutDetector"; }
    @Override public SignalType signalType() { return SignalType.VWAP_BREAKOUT; }
